#!/usr/bin/env bash
# Shell script to train VQ-VAE-2 model for ECG generation
#
# Usage:
#   ./run_train_vqvae2.sh [fit|test|sample]
#
# Examples:
#   ./run_train_vqvae2.sh fit                           # Train VQ-VAE-2
#   ./run_train_vqvae2.sh test path/to/checkpoint.ckpt  # Test model
#   ./run_train_vqvae2.sh sample path/to/checkpoint.ckpt # Generate samples
...
train_vqvae2() {
    print_header "Training VQ-VAE-2"
...
sample_vqvae2() {
    ...
    print_header "Sampling from VQ-VAE-2"
...
print_header "VQ-VAE-2 Training Script"

# Check if vqvae2.py exists
if [ ! -f "vqvae2.py" ]; then
...
case "$COMMAND" in
    ...
    test|eval)
        CKPT_PATH="${2:-}"
        test_vqvae2 "$CKPT_PATH"
        ;;
    sample|generate)
        CKPT_PATH="${2:-}"
        sample_vqvae2 "$CKPT_PATH"
        ;;
    *)
        ...
        echo ""
        echo "Commands:"
        echo "  fit                           Train VQ-VAE-2 model"
        echo "  test <checkpoint_path>        Test a trained model"
        echo "  sample <checkpoint_path>      Generate samples from a trained model"
        echo ""
        echo "Environment Variables:"
        echo "  DATA_DIR                      Path to MIMIC-IV-ECG dataset"
        echo "  EXP_NAME                      Experiment name (default: vqvae2_mimic)"
        echo "  BATCH_SIZE                    Batch size (default: 32)"
        echo "  MAX_EPOCHS                    Maximum epochs (default: 200)"
        echo "  LR                            Learning rate (default: 0.0003)"
        echo "  HIDDEN_CHANNELS               Hidden channels (default: 128)"
        echo "  N_EMBEDDINGS_TOP              Top codebook size (default: 512)"
        echo "  N_EMBEDDINGS_BOT              Bottom codebook size (default: 512)"
        ...
        echo "  $0 test runs/vqvae2_mimic/seed_42/checkpoints/last.ckpt"
        echo "  N_SAMPLES=32 $0 sample runs/vqvae2_mimic/seed_42/checkpoints/last.ckpt"
        exit 1
        ;;
esac
